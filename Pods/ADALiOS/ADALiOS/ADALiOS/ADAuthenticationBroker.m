...
- (void)webAuthenticationDidCompleteWithURL:(NSURL *)endURL
{
    ...
    {
        // Prasad, Preeti
        // After successful login in azure, while dismissing the webview, if main thread calls some other function ie viewDidAppear from Application code the completion block was getting lost because it was scheduled in background thread.
        // So called [self dispatchCompletionBlock:nil URL:endURL] in the same thread dismiss view is getting called
        // Change start
        // Dismiss the authentication view and dispatch the completion block
        [parentController dismissViewControllerAnimated:YES completion:^{
