...
{
    return [CLImageEditorTheme localizedString:@"CLTextTool_DefaultTitle" withDefault:@""];
}
...
- (void)addNewText
{
    _CLTextView *view = [[_CLTextView alloc] initWithTool:self withFrame:CGRectZero];
...
const CGFloat MAX_FONT_SIZE = 28.0;
...
{
    self = [super initWithFrame:CGRectMake(0, 200, 132, 132)];
    if(self){
        ...
        _label.contentScaleFactor = 1/MAX_FONT_SIZE;
        _label.textAlignment = NSTextAlignmentCenter;
        _label.delegate = self;
        _label.returnKeyType = UIReturnKeyDone;
        self.text = @"";
        [_label setText:@""];
        [_label setAutocorrectionType:UITextAutocorrectionTypeNo];
        [self addSubview:_label];
        
        CGSize size = [_label sizeThatFits:CGSizeMake(FLT_MAX, FLT_MAX)];
        _label.frame = CGRectMake(16, 16, frame.size.width, size.height );
        self.frame = CGRectMake(0, 200, frame.size.width + 32, size.height + 32);
...
{
    _label.tintColor = borderColor;
}

- (UIColor*)borderColor
{
    return _label.tintColor;
}
...
- (void)setText:(NSString *)text
{
    if(![text isEqualToString:_text]){
        _text = text;
        _label.text = (_text.length>0) ? _text : [CLImageEditorTheme localizedString:@"CLTextTool_EmptyText" withDefault:@""];
    }
}
...
- (void)pushedDeleteBtn:(id)sender
{
    ...
    [self resignResponderFromCurrentContext];
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text{
    ...
    //int g = textView.text.length;
    if ([text isEqualToString:@"\n"]) {
        [self resignResponderFromCurrentContext];
    } else if(text.length>0 &&textView.text.length>=12){
         return NO;
    }
    return YES;
}
...
- (void) resignResponderFromCurrentContext {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"DoneButtonPressed" object:nil];
}
- (void)textViewDidBeginEditing:(UITextView *)textView{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"StartTextEditingPressed" object:nil];
}
