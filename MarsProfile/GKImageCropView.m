...
- (void)layoutSubviews{
    ...
    CGFloat yOffsetForScrollView;
    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad){
        yOffsetForScrollView = -32;
    }else{
        yOffsetForScrollView = -45;
    }
    self.scrollView.frame = CGRectMake(xOffset, yOffsetForScrollView, size.width, size.height);
    self.scrollView.contentSize = CGSizeMake(size.width, size.height);
    self.imageView.frame = CGRectMake(0, -30, width, height);
}
